...
router.get('/', authenticate, async (req, res) => {
    const userId = req.userId;
    const page = parseInt(req.query.page, 10) || 1;
    const limit = parseInt(req.query.limit, 10) || 10; // ✅ Convert limit to number
    const offset = (page - 1) * limit;

    console.log(`Fetching expenses for userId: ${userId}, Page: ${page}, Limit: ${limit}, Offset: ${offset}`);
    ...
    try {
        ...
        // ✅ Fix: Use string interpolation instead of placeholders for LIMIT & OFFSET
        const query = `
            SELECT e.id, e.user_id, e.amount, e.income, e.description, 
                   e.category, e.created_at, u.is_premium
            FROM expenses e
        ...
        const results = await executeQuery(query, [userId]); // ✅ Only userId as parameter now
